...
fn main() {
	...
	if d < 0.0 {
		...
		if c < 0.0 {
			...
			println!("\x1b[92mДискриминант равен нулю. Единственный корень: {}.\x1b[0m", c.abs());
		}
		else {
		...
		if c1 == 0.0 { print!("{}", c1.abs()); }
		if c1 > 0.0 { print!("{}, {}", f64::sqrt(c1), -1.0 * f64::sqrt(c1)); }
		if c1 >= 0.0 && c2 >= 0.0 { print!(" и ") }
		if c2 == 0.0 { print!("{}", c2.abs()); }
